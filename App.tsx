...
import { getPrioritizedRecommendations, ScoredRecommendation } from './services/recommendationService';
    ...
    <div className="max-w-4xl mx-auto flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-emerald-100 opacity-80 uppercase tracking-widest font-semibold">Expertise Biodiversité & Sites</p>
        </div>
      </div>
...
const Questionnaire = ({ onComplete }: { onComplete: (data: UserResponses) => void }) => {
  ...
  const visibleFields = useMemo(() => {
    return QUESTIONNAIRE.fields.filter(f => {
      ...
      return op === '==' ? responses[field] === value : responses[field] !== value;
    });
  }, [responses]);
    ...
    <div className="max-w-2xl mx-auto my-12 p-8 bg-white rounded-2xl shadow-xl border border-slate-100">
      <div className="mb-8">
        <div className="flex justify-between items-end mb-2">
          <span className="text-xs font-bold text-emerald-700 uppercase tracking-wider">Étape {currentStep + 1} sur {visibleFields.length}</span>
          <span className="text-xs text-slate-400">{Math.round(progress)}% complété</span>
        </div>
        ...
        {currentField.type === 'select' ? (
          <div className="grid gap-3">
            {currentField.options?.map(opt => (
              <button
                key={opt.value}
                onClick={() => { handleValueChange(opt.value); handleNext(); }}
          ...
          <div className="flex flex-col gap-4">
            <input
              ...
              onKeyDown={(e) => e.key === 'Enter' && responses[currentField.key] && handleNext()}
      ...
      <div className="mt-12 flex items-center justify-between text-slate-400">
        ...
        <div className="text-[10px] uppercase font-medium">Référentiel AFILOG / BREEAM / HQE</div>
      </div>
    </div>
  );
};
...
const RiskGauge = ({ score }: { score: number }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center">
      <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Indice de Risque Projet</h3>
      <div className="relative w-24 h-24 mb-3">
         <svg className="w-full h-full" viewBox="0 0 36 36">
            ...
            <path className={`transition-all duration-1000 ${score < 30 ? 'stroke-emerald-500' : score < 60 ? 'stroke-orange-500' : 'stroke-red-500'}`} strokeWidth="3" strokeDasharray={`${score}, 100`} strokeLinecap="round" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
         </svg>
         <div className={`absolute inset-0 flex flex-col items-center justify-center font-bold text-xl ${getRiskColor(score)}`}>
           {score}%
         </div>
      </div>
      <span className={`text-sm font-black uppercase ${getRiskColor(score)}`}>{getRiskLabel(score)}</span>
    </div>
  );
};
...
const Results = ({ responses }: { responses: UserResponses }) => {
  ...
  const results = useMemo(() => getPrioritizedRecommendations(RECOMMENDATIONS, responses), [responses]);

  React.useEffect(() => {
    const fetchSynthesis = async () => {
      try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const personaLabel = QUESTIONNAIRE.fields[0].options?.find(o => o.value === responses.persona)?.label || responses.persona;
        const assetLabel = QUESTIONNAIRE.fields[3].options?.find(o => o.value === responses.asset_type)?.label || responses.asset_type;
        
        const prompt = `Génère une synthèse courte et professionnelle (3-4 phrases) pour ce profil.
        Persona: ${personaLabel}
        Type de site: ${assetLabel}
        Stade: ${responses.project_stage || 'N/A'}
        Notes: ${responses.site_notes || 'N/A'}
        Risque calculé: ${results.projectRiskScore}%
        Ton: Professionnel, valorisant les enjeux biodiversité comme levier stratégique.`;

        const response = await ai.models.generateContent({
          model: 'gemini-3-flash-preview',
          contents: prompt,
        });
        setSynthesis(response.text || "Profil qualifié. Prêt pour les recommandations.");
      } catch (e) {
        setSynthesis("Synthèse du profil prête. Voici vos recommandations priorisées.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="max-w-2xl mx-auto my-32 text-center">
        ...
        <p className="text-slate-600 font-medium">Analyse des données et calcul du score de pertinence...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto my-12 px-4 pb-24">
        ...
        <section className="lg:col-span-3 bg-emerald-900 text-white p-8 rounded-3xl shadow-xl relative overflow-hidden">
          <div className="relative z-10">
            <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
              <i className="fa-solid fa-id-card-clip text-emerald-400"></i> Synthèse Stratégique
            </h2>
            <p className="text-emerald-50 leading-relaxed text-lg italic">"{synthesis}"</p>
            <div className="mt-6 flex flex-wrap gap-2">
              ...
              <span className="bg-emerald-700/50 px-3 py-1 rounded-lg text-xs font-semibold border border-emerald-500/30 uppercase tracking-wider">
                {responses.certif_target !== 'non' ? responses.certif_target : 'Pas de certification'}
              </span>
            </div>
          </div>
      ...
      <section className="mb-16">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <h2 className="text-2xl font-extrabold text-slate-800 flex items-center gap-3">
            <span className="bg-emerald-600 text-white w-8 h-8 rounded-full flex items-center justify-center text-sm italic">8</span>
            Top 8 Recommandations Priorisées
          </h2>
          <div className="text-xs text-slate-400 italic">Classées par score de pertinence calculé pour votre profil</div>
        </div>
        
        <div className="grid md:grid-cols-2 gap-6">
          {results.top8.map(({ rec, score }, idx) => (
            <div key={rec.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow group flex flex-col">
              ...
              <h3 className="font-bold text-lg text-slate-900 mb-3 group-hover:text-emerald-700 transition-colors leading-tight">{rec.titre}</h3>
              <p className="text-sm text-slate-600 mb-6 leading-relaxed flex-grow">{rec.description}</p>
              
              <div className="grid grid-cols-2 gap-4 text-xs pt-4 border-t border-slate-50">
                <div>
                  <h4 className="font-bold text-slate-800 uppercase text-[9px] mb-1">Niveau de risque</h4>
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${
                      rec.niveau_risque === 'faible' ? 'bg-green-500' :
                      rec.niveau_risque === 'moyen' ? 'bg-orange-500' : 'bg-red-500'
                    }`}></span>
                    <span className="capitalize text-slate-500 font-medium">{rec.niveau_risque}</span>
                  </div>
                </div>
                <div>
                  <h4 className="font-bold text-slate-800 uppercase text-[9px] mb-1">Horizon</h4>
                  <span className="text-slate-500 flex items-center gap-1 italic">
                    <i className="fa-solid fa-clock opacity-50"></i> {rec.horizon}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-2">
          <section className="mb-12">
            <h2 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
              <i className="fa-solid fa-plus-circle text-emerald-600"></i> Actions complémentaires
            </h2>
            <div className="space-y-4">
              {results.complementary.map(({ rec, score }) => (
                <div key={rec.id} className="bg-slate-50 p-4 rounded-xl flex items-start gap-4 hover:bg-white border border-transparent hover:border-slate-200 transition-all">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-900 text-sm mb-1">{rec.titre}</h4>
        ...
        <div>
          <section className="mb-12 bg-orange-50 p-6 rounded-2xl border border-orange-100">
            <h2 className="text-lg font-bold text-orange-900 mb-4 flex items-center gap-2">
              <i className="fa-solid fa-triangle-exclamation text-orange-600"></i> Points de vigilance
            </h2>
            <div className="space-y-4">
              {results.vigilance.length > 0 ? results.vigilance.map(({ rec, score }) => (
                <div key={rec.id} className="text-sm bg-white/50 p-3 rounded-lg border border-orange-200/50">
                  <h4 className="font-bold text-orange-800 mb-1 flex justify-between items-center">
                    <span>{rec.titre}</span>
                    <span className="text-[10px] bg-orange-200 text-orange-800 px-1.5 rounded">{score}</span>
                  </h4>
                  <p className="text-orange-700/80 text-xs leading-relaxed">{rec.description}</p>
                </div>
              )) : (
                <p className="text-xs text-orange-600 italic">Aucune vigilance critique identifiée selon vos réponses.</p>
              )}
            </div>
          </section>

          <section>
            <h2 className="text-lg font-bold text-slate-800 mb-4">Sources et références</h2>
            <ul className="space-y-3">
              {[...results.top8, ...results.complementary].map(({ rec }, i) => (
                <li key={i} className="text-[10px] text-slate-400 leading-tight flex gap-2">
                  <span className="font-bold text-slate-300">[{rec.id}]</span> {rec.source}
                </li>
              ))}
            </ul>
          </section>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 p-4 shadow-2xl z-40 md:static md:bg-transparent md:border-none md:shadow-none md:mt-20">
        ...
        {step === 'intro' && (
          <div className="max-w-4xl mx-auto my-20 px-6 text-center">
            ...
            <h1 className="text-4xl md:text-6xl font-black text-slate-900 mb-8 leading-[1.1]">
              Qualifiez la biodiversité de votre site en <span className="text-emerald-600 underline decoration-emerald-200 underline-offset-8">5 minutes</span>.
            </h1>
            <p className="text-xl text-slate-500 mb-12 max-w-2xl mx-auto leading-relaxed">
              Coach Biodiversité PRO analyse vos contraintes métiers et techniques pour vous proposer une trajectoire sur-mesure conforme aux référentiels 2025.
            </p>
            <div className="flex flex-col md:flex-row items-center justify-center gap-4">
              ...
              <div className="text-slate-400 text-sm flex items-center gap-4">
                <span className="flex items-center gap-1"><i className="fa-solid fa-check text-emerald-500"></i> Gratuit</span>
                <span className="flex items-center gap-1"><i className="fa-solid fa-check text-emerald-500"></i> Sans compte</span>
                <span className="flex items-center gap-1"><i className="fa-solid fa-check text-emerald-500"></i> Conforme BREEAM</span>
              </div>
            </div>
      ...
      <footer className="bg-slate-50 border-t border-slate-200 py-12 px-4 mt-20">
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2 opacity-50">
            ...
            <span className="font-bold text-slate-800">Coach Biodiversité PRO</span>
          </div>
          <div className="text-xs text-slate-400 text-center md:text-right">
            © 2025 — Développé pour les acteurs de l'immobilier durable.<br />
            Les recommandations sont indicatives et ne remplacent pas un audit écologique sur site.
          </div>
        </div>
      </footer>
    </div>
